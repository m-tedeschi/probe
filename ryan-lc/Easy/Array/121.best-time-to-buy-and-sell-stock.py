...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        # Time: O(n), Space: O(1)
        # Initialization: prices[0] is the minimum price we know
        # Max profit starts at 0
        min_price_so_far = prices[0]
        max_profit = 0

        # For each price, calculate the profit we can make so far
        # And update the max_profit and min_price_so_far running variables
        for price in prices:
